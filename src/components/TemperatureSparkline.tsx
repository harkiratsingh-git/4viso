...
export const TemperatureSparkline: React.FC<TemperatureSparklineProps> = ({
  ...
  const y = (temp: number) => height - ((temp - lo) / span) * height;
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="flex-shrink-0" aria-hidden="true">
      {bandBottom > bandTop && (
        <rect x={0} y={bandTop} width={width} height={bandBottom - bandTop} fill="currentColor" className={t.light ? 'text-emerald-500/15' : 'text-emerald-500/10'} />
      )}
      <path
        d={linePath}
        fill="none"
        stroke="currentColor"
        strokeWidth={1.25}
        className={isExcursion ? (t.light ? 'text-rose-600' : 'text-rose-400') : t.light ? 'text-slate-400' : 'text-slate-500'}
      />
      <circle
        ...
        r={1.75}
        fill="currentColor"
        className={isExcursion ? (t.light ? 'text-rose-600' : 'text-rose-400') : t.light ? 'text-slate-500' : 'text-slate-400'}
      />
    </svg>
  );
};
