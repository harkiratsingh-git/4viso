 ...
 *
 * Always dark, deliberately — this lives inside the Add Lane wizard, which (like everything
 * outside the Simple Dashboard) is locked to a fixed dark theme regardless of the app-wide
 * theme preference, so it can't end up light while the rest of the wizard stays hardcoded dark.
 */
export const SimpleRouteRecommendation: React.FC<SimpleRouteRecommendationProps> = ({ options }) => {
  ...
  if (!recommended) {
    return (
      <div className="p-5 rounded-xl border flex items-center gap-2 text-xs bg-slate-950/70 border-slate-800 text-slate-400">
  ...
  return (
    <div className="p-4 rounded-xl border bg-emerald-950/20 border-emerald-800/50">
      <div className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider mb-2 text-emerald-400">
      ...
      <div className="flex items-center flex-wrap gap-1 text-sm font-mono mb-3">
        {recommended.waypoints.map((w, i) => (
          <React.Fragment key={`${w.iata}-${i}`}>
            {i > 0 && <ArrowRight className="w-3.5 h-3.5 text-slate-600" />}
            <span className="px-2 py-0.5 rounded border bg-slate-900 border-slate-700 text-slate-200">{w.iata}</span>
          </React.Fragment>
        ))}
      </div>
      ...
      {recommended.legs[0]?.topCarrierPick && (
        <p className="text-sm text-slate-300">
          Carrier: <strong className="text-white">{recommended.legs[0].topCarrierPick.carrier.name}</strong>
      ...
      <div className="mt-3 pt-3 border-t flex items-center gap-4 text-xs border-emerald-900/40 text-slate-400">
