// Server-side .docx summary report generator. Runs inside the Edge Function, never in the
// browser — the frontend only ever receives a signed download link once this has finished.
// Every number in the summary table comes straight from the `dashboard_summary` view (never
// recomputed here), exactly like every other part of this app. Verbatim copy of
// ../assistant/report.ts — this function is a separate Supabase Edge Function deploy unit (own
// files array), so it can't import across function directories; keep both in sync manually.
